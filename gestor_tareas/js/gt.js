document.getElementById('taskDeadline').setAttribute('min', new Date().toISOString().split('T')[0]);
        ...
        document.getElementById('taskDeadlineTime').value = new Date().toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' }); // Hora actual por defecto

        // Establecer foco en el campo de texto al cargar la página
        window.onload = () => {
            document.getElementById('taskName').focus();
            loadTasksFromLocalStorage(); // Cargar tareas desde localStorage
        };
        ...
        function loadTasksFromLocalStorage() {
            ...
            savedTasks.forEach(task => {
                ...
                taskElement.innerHTML = `
                    ...
                    <div class="dates">
                        <span>Creado: ${task.createdDate} ${task.createdTime}</span><br>
                        <span>Plazo: ${task.deadlineDate} ${task.deadlineTime}</span>
                    </div>
                    <div class="actions">
                ...
                const list = document.getElementById(task.status).querySelector('.task-list');
                list.appendChild(taskElement);
                sortTasks(list); // Ordenar tareas por prioridad
            });
            checkDeadlines(); // Verificar plazos después de cargar las tareas
        }
        ...
        function addTask() {
            ...
            saveTasksToLocalStorage(); // Guardar tareas después de añadir
        }
        ...
        function deleteTask(button) {
            ...
            taskElement.remove();
            saveTasksToLocalStorage(); // Guardar tareas después de eliminar
        }
        ...
        function changeTaskStatus(button) {
            ...
            saveTasksToLocalStorage(); // Guardar tareas después de cambiar estado
        }
